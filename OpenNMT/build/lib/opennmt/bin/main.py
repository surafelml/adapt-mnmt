...
def main():
  ...
  # to-do: by pass the runner.py class and call a new train_with_freezing.py, load additional config file (config_freeze_update.yml),
  # TODO: in train_with_freezing add opts for arch-component selection and freeze/update enc, dec, emb (src, tgt, tied), softmax.
  #if args.run == "freeze_train_and_eval": # add option to --run arg
  #  runner.freeze_train_and_eval(checkpoint_path=args.checkpoint_path)
  #elif
  if args.run == "train_and_eval":
