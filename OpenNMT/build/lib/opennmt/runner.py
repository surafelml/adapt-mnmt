...
class Runner(object):
  ...
  def _build_train_spec(self, checkpoint_path):
    ...
    if checkpoint_path is not None and "load_weights" in self._config:
      ...
      loadw = self._config["load_weights"] # search in the var_list with [0] for optim..., [1] for enc/dec and [1:2] for embs, projection 
      if loadw is not None:
        ...
        if not loadw.get("optim"): #if avoided the the next two are avoided
      ...
      train_hooks.append(hooks.LoadWeightsFromCheckpointHook(checkpoint_path, not_restore))  #self._config["load_partial_weights"]))
  ...
  def _build_eval_spec(self):
    ...
    eval_spec = tf.estimator.EvalSpec(
        ...
        start_delay_secs=self._config["eval"]["start_delay_secs"],
  ...
  #TODO: proceed with this one if passing arguments to freeze directly from config-to-optim doesn't work
  """ 
  def freeze_train_and_eval(self, checkpont_path=None): # NEW: if not working with warm_start move to a new train/runner_update_freeze.py
      Loads function to load selected params, Runs the training and evaluates.
        - possible strategies:
        -- load model vars, select trainable vars and pass var_list to custom optimizers
        -- ???
      :param checkpont_path:
      :return:
    #tf.Variable(weights, trainable=False)
    #x = tf.select(x, tf.stop_gradient(x), x)

    # in a different way than warm_start (i will use this maybe with DV), are:
    # - i. passing var_list to the tf.train.OPT
    # - ii. tf.Variable (weights, trainable=False) for emb's, enc, dec
    # - iii. ? find for optimized way.
    tf.get_default_graph().get_collection_ref(tf.GraphKeys.TRAINABLE_VARIABLES) #list of names
    #good intution:
    ...
    # resume by checking model.py _model_fun() to understand how the params for optim is passed
    """

  # TODO: load only sub-components from init (such as, encoder, decoder, shared_emb, softmax/output_layer)
  """
  def train_after_loading_sub_components(self, check):

  Runs the training and evaluation loop after loading specific components from init model 
  and initializing the rest with random weights
  
  Args: 
    build new _build_train_spec or modify the existing one.
  """
