...
def _update_vocabulary_variables(output_dir,
  ...
  for name, tensor in six.iteritems(variables):
    if scope_name in name and any(d == current_size for d in tensor.shape):
      tf.logging.debug("Updating variable %s" % name)   # TODO: not shown
      variables[name] = _update_vocabulary_variable(tensor, current_size, mapping) # TODO: visualize the pre-post update network components & weights
      tf.logging.info("Updated variable %s" % name) # TODO: visualize the updated variable values variables[name]
...
def update_vocab(model_dir,
  ...
  if mode not in ("merge", "replace"):
    raise ValueError("invalid vocab update mode: %s" % mode) # TODO: mode-initialize-opts

  if new_src_vocab is None and new_tgt_vocab is None and new_shared_vocab is None: # one of this should exist to proceed.
    return model_dir
  ...
  # TODO: three update types, i. src ii. tgt iii. src & tgt iv. single shared, what if? ||| do shared once else do src or tgt or stc&tgt
  #if shared_embeddings == EmbeddingsSharingLevel.SOURCE_TARGET_INPUT and new_shared_vocab \
  #        is not None and current_shared_vocab is not None:
  if new_shared_vocab is not None: # here we only expand the scope, to include shared_embs, it will get it by dim size of tensors
    tf.logging.info("Updating shared embeddings...")
    _update_vocabulary_variables(
        output_dir, variable_value, current_shared_vocab, new_shared_vocab, "transformer", mode)
  else:
    if new_src_vocab is not None: #and shared_embeddings == EmbeddingsSharingLevel.NONE: # uncessary condition
      tf.logging.info("Updating src side embeddings...")
      _update_vocabulary_variables(
          output_dir, variable_value, current_src_vocab, new_src_vocab, "encoder", mode)
    if new_tgt_vocab is not None: #and shared_embeddings == EmbeddingsSharingLevel.NONE:
      tf.logging.info("Updating tgt side embeddings...")
      _update_vocabulary_variables(
...
# COPY TEST SETS HERE, SINCE IT CONTRAIN MODEL VOCAB,  #test-sets/$DOMAIN/test.src/tgt #ENABLE ENCODING & VOCAB ?fig=session_config)
