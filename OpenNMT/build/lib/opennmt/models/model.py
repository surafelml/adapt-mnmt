...
class Model(object):
  ...
  def _get_variables(self, params):
    """Returns the list of variables, from the trainable variable=True, to be optimized.

    Args: A list of network sub-components to return.
    TODO: avoid looping using list comprehension with a direct variables to optimize
    Returns:
      The list of variables.
    """
    #todo: VARLIST NAMING PARAMS INDEPENDENT EITHER CALL THE SCOPE TO MAP THE ENCODER/DECODER/EMBS OR ADD NEW PARAM TO DINTINGUIS TO PUT transformer/seq2seq
    tmp_list = [] # GET EH SCOPE FOR EACH COMPONENT NOT HARDCODED SINCEE IT INCLUEES RNN MODEL TOO
    if not params.get("encoder"):
        tmp_list.append("transformer/encoder")
    if not params.get("decoder"):
        tmp_list.append("transformer/decoder")
    if not params.get("shared_embeddings"):  # get the not shared once too
        tmp_list.append("transformer/shared_embeddings")
    if not params.get("softmax"):
        tmp_list.append("transformer/decoder/dense")

    def _is_load_weights(variable):
        for name in tmp_list:
            return variable.name.startswith(name)
    var_list = [v for v in tf.trainable_variables() if _is_load_weights(v)]  # if v is in tmp_list]

    tf.logging.info("TRAINABLE VARIABLES: %s", tf.trainable_variables())
                    #json.dumps(tf.trainable_variables(), indent=3, sort_keys=True))

    tf.logging.info("TRAINABLE VARIABLES TO OPTIMIZE:  %s", var_list)
                    #json.dumps(var_list, indent=3, sort_keys=True))
    return var_list
