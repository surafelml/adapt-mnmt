...
class Model(object):
  ...
  def _get_variables(self, params):
    ...
    tf.logging.info("Freezing model components: %s", 
    ...
    '''
    if not params.get("shared_embs"): 
        tmp_list.append("transformer/shared_embeddings")
    if not params.get("src_embs"): 
        tmp_list.append("transformer/encoder/w_embs")
    if not params.get("tgt_embs"): 
        tmp_list.append("transformer/decoder/w_embs")
    if not params.get("projection"):
        tmp_list.append("transformer/decoder/dense")
    '''

    tf.logging.info("Trainable model component/variable list: %s",
                     json.dumps(tmp_list, indent=2, sort_keys=True))

    def _is_load_weights(variable):
        for name in tmp_list:
            return variable.name.startswith(name)

    var_list = [v for v in tf.trainable_variables() if _is_load_weights(v)]

    #tf.logging.info("ALL TRAINABLE VARIABLES: %s", tf.trainable_variables())
