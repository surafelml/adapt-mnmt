...
class LoadWeightsFromCheckpointHook(tf.train.SessionRunHook):
  """"Hook that loads model variables from checkpoint before starting the training.
    optionally - allows to load specifi model components.
 
  """
  def __init__(self, checkpoint_path, not_restore):
    self.checkpoint_path = checkpoint_path
    self.not_restore = not_restore # fastest hack for model vars to avoid loading, TODO: cln-simplify 
  ...
  def begin(self):
    ...
    for name, _ in var_list: 
      ...
      if (len(name.split('/')) >= 2):
        ...
        # check if embs & projec should be included with the enc-dec
        embs_proj = ""
        if (len(name.split('/')) >= 3):
