...
def freeze_update_op(params):
    # TODO: stage1: defining 4 components to freeze or update, stage2: layer and attention head wise freezing/updating
    # TODO: move it to a new method in a diff/here and call it as [ freeze_update_fn(): ]
    # TODO: layer-by-layer freezing simply "encoder/layer_X?" or encoder/layer_5/multi_head/X? or discriminator/X? to shape to a different dialect
    #freeze = params.get("freeze")
    var_list = []
    for variable in tf.trainable_variables():
        if params["encoder"] == False:
            var_list.append(variable.name.starswith("encoder"))
        if params["decoder"] == False:
            var_list.append(variable.name.starswith("decoder"))
        if params["shared_embeddings"] == False:
            var_list.append(variable.name.starswith("transformer/shared_embeddings"))
        if params["softmax"] == False: # softmax         #TODO: softmax, only for source & target embeddings
            var_list.append(variable.name.starswith("transformer/decoder/dense")) #variable.name.endswith #TODO: check regularization part to filter weights
        #if params["src_embeddings"] == False:
        #    var_list.append(variable.name.starswith("encoder/w_embs"))
        #if params["tgt_embeddings"] == False:
        #    var_list.append(variable.name.starswith("decoder/w_embs"))
    tf.logging.info("VARIABLE_LIST TO OPTIMIZE: ", var_list)
    return var_list
...
"""
    if freeze is None:
        ...
        if freeze[] == "encoder":  # "transformer/encoder" #TODO: what if we want to freeze all but one
            for variable in tf.trainable_variables():
                if not variable.name.starswith("encoder"):  #
                    var_list.append(variable)
        elif freeze == "decoder":
            for variable in tf.trainable_variables():
                if not variable.name.starswith("decoder"):
                    var_list.append(variable)
        elif freeze == "embs":  # shared_embeddings
            for variable in tf.trainable_variables():
                if not variable.name.starswith("embs"):
                    var_list.append(variable)
        elif freeze == "softmax":
            for variable in tf.trainable_variables():
                if not variable.name.starswith("softmax"):
                    var_list.append(variable)
        else:
            var_list = None
...
def regularization_penalty(regularization_type, scale, weights_list=None):
  ...
  def _is_bias(variable):
    return len(variable.shape.as_list()) == 1 and variable.name.endswith("bias:0") #TODO: what is this, can we apply it in extreme adaptation for shared embs?
