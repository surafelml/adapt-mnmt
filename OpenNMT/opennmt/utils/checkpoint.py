...
def _update_vocabulary_variables(output_dir,
  ...
  for name, tensor in six.iteritems(variables):
    if scope_name in name and any(d == current_size for d in tensor.shape):
      tf.logging.info("Updating Variable %s" % name)
      variables[name] = _update_vocabulary_variable(tensor, current_size, mapping, init) 
...
def update_vocab(model_dir,
  ...
  Args:
    ...
    init: new voca embs initialization mode.
    session_config: Optional configuration to use when creating the session.
  ...
  if init not in ("zeros", "random"):
    raise ValueError("invalid vocab initialization mode: %s" % init) 
  if new_src_vocab is None and new_tgt_vocab is None and new_shared_vocab is None: 
    return model_dir 
  checkpoint_path = tf.train.latest_checkpoint(model_dir)
  if checkpoint_path is None:
    raise ValueError("No checkpoint found in directory %s" % model_dir)

  tf.logging.info("Updating vocabulary related variables with mode: %s, initialization: %s, in checkpoint %s" % (mode, init, checkpoint_path))
  variable_value = get_checkpoint_variables(checkpoint_path)
  if not tf.gfile.Exists(output_dir):
    tf.gfile.MakeDirs(output_dir)

  # given how the naming of variable and scope in checkpoint, reconfigure the vocab_update in to 
  # only shared embs, src embs, tgt embs or src & tgt embs 
  if new_shared_vocab is not None: 
    tf.logging.info("Updating shared vocabulary relaed variables ...")
    _update_vocabulary_variables(
        output_dir, variable_value, current_shared_vocab, new_shared_vocab, "transformer", mode, init)
  else:
    if new_src_vocab is not None: 
      tf.logging.info("Updating source vocabulary related variables ...")
      _update_vocabulary_variables(
          output_dir, variable_value, current_src_vocab, new_src_vocab, "encoder", mode, init)

    if new_tgt_vocab is not None: 
      tf.logging.info("Updating target vocabulary related variables ...")
      _update_vocabulary_variables(
          output_dir, variable_value, current_tgt_vocab, new_tgt_vocab, "decoder", mode, init)
